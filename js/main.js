...
function loadAllCards() {
  ...
  xhr.onload = () =>
    xhr.status == 200 ? createCards(JSON.parse(xhr.responseText)) : "";
  xhr.send();
}
//Load Card
function loadCard(data, par) {
  ...
  if (par == "One way") {
    xhr.open(
      "POST",
      "http://ramin000-001-site1.dtempurl.com/api/User/search",
      true
    );
  } else {
    xhr.open(
      "POST",
      "http://ramin000-001-site1.dtempurl.com/api/User/twowaysearch",
      true
    );
  }
...
function addEvents() {
  ...
  baby.addEventListener("input", () => {
    calculate(
      ...
      children.value,
      adult.value,
      locations[popup.getAttribute("name") - 1]
    );
...
function updatePopUp(id) {
  ...
  flightTime.textContent =
    data.flightTime.slice(0, 4) +
    "-" +
    data.flightTime.slice(5, 7) +
    "-" +
    data.flightTime.slice(8, 10);
  //Bag
...
function calculate(baby_value, children_value, adult_value, data) {
  let sum = 0;
  let _amount =
    Number(baby_value) + Number(children_value) + Number(adult_value);
  sum +=
    baby_value * data.ticketPriceForBaby +
    children_value * data.ticketPriceForChildren +
    adult_value * data.ticketPriceForAdult;
  if (_amount > 0) {
    addToLocal(
      JSON.stringify({
        _totalPrice: sum,
...
function inceaseAndDecrease(par) {
  ...
  if (par.classList.contains("minus")) {
    ...
    if (
      baby + children + adult <
      locations[popup.getAttribute("name") - 1].ticketAmount
    ) {
      ++count;
    }
  }
